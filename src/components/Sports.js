...
export const Sports = ({ OpenArticle }) => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `https://newsapi.org/v2/top-headlines?country=in&category=sports&apiKey=YOURAPIKEY`
        );
  ...
  const fetchSearchData = async (Query) => {
    try {
      const response = await fetch(
        `https://newsapi.org/v2/top-headlines?q=${Query}&category=sports&apiKey=YOURAPIKEY`
      );
      const jsonData = await response.json();
      setData(jsonData);
    } catch (error) {
