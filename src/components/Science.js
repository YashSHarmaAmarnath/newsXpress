...
export const Science = ({ OpenArticle }) => {
const [data, setData] = useState({ articles: [] });
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `https://newsapi.org/v2/top-headlines?country=in&category=science&apiKey=YOURAPIKEY`
        );
  ...
  const fetchSearchData = async (Query) => {
    try {
      const response = await fetch(
        `https://newsapi.org/v2/top-headlines?q=${Query}&category=science&apiKey=YOURAPIKEY`
      );
      const jsonData = await response.json();
      setData(jsonData);
    } catch (error) {
